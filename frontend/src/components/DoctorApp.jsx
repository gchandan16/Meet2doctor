export default function DoctorApp() {
  return (
<div id="app_section">
			<div className="container">
				<div className="row justify-content-around">
					<div className="col-md-5">
						<p><img src="img/app.png" alt="" className="img-fluid" width="400" height="350" /></p>
					</div>
					<div className="col-md-6">
						<small style={{color:"Black"}}>Application</small>
						<h3 style={{color:"Black"}}>Download <strong style={{color:"Blue"}}>Meet2Doctor App</strong></h3>
						<p className="lead" style={{color:"Black"}}>Book doctor appointments online and manage your health records securely on the go. Your peace of mind is just a download away.</p>
						<div className="app_buttons wow" data-wow-offset="100">
							<svg version="1.1" xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink" x="0px" y="0px" viewBox="0 0 43.1 85.9" style={{enableBackground:"new 0 0 43.1 85.9"}} xmlSpace="preserve">
							<path strokeLinecap="round" strokeLinejoin="round" className="st0 draw-arrow" d="M11.3,2.5c-5.8,5-8.7,12.7-9,20.3s2,15.1,5.3,22c6.7,14,18,25.8,31.7,33.1" />
							<path strokeLinecap="round" strokeLinejoin="round" className="draw-arrow tail-1" d="M40.6,78.1C39,71.3,37.2,64.6,35.2,58" />
							<path strokeLinecap="round" strokeLinejoin="round" className="draw-arrow tail-2" d="M39.8,78.5c-7.2,1.7-14.3,3.3-21.5,4.9" />
						</svg>
							<a href="#0" className="fadeIn"><img src="img/apple_app.png" alt="" width="150" height="50" data-retina="true" /></a>
							<a href="#0" className="fadeIn"><img src="img/google_play_app.png" alt="" width="150" height="50" data-retina="true" /></a>
						</div>
					</div>
				</div>
			
			</div>
			
		</div>

  );
}